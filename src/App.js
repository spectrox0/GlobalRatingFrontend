...
export default function App() {
  ...
  const getGeoInfo = async () => {
    try {
      ...
      const newRes = await data.paisesActivos.find(
        element => element.abreviatura === res
      );
      if (newRes) {
        localStorage.setItem("country", newRes.abreviatura);
        return setCountry({
          value: newRes.abreviatura,
  ...
  React.useEffect(() => {
    if ((data && !loading) || error) {
      if (error) {
        localStorage.setItem("country", "ve");
        return setCountry({ value: "ve", nombre: "venezuela" });
      } else if (data.paisesActivos.length > 1) getCountry();
      else {
        localStorage.setItem("country", "ve");
        return setCountry({ value: "ve", nombre: "venezuela" });
      }
  ...
  const getCountry = async () => {
    ...
    if (res) {
      const newRes = data.paisesActivos.find(
        element => element.abreviatura === res
      );
      if (newRes) {
        localStorage.setItem("country", newRes.abreviatura);
  ...
  return (
    <BrowserRouter>
      ...
      {country ? (
        <>
          {" "}
          <Routes country={country.value} /> <DarkFooter />{" "}
        </>
      ) : (
        <Loading />
      )}
